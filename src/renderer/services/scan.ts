...
export function getAllDirectoryList({path}) {
  ...
  while (queue.length !== 0) {
    ...
    const directoryFiles: { path: string, dirent: Dirent }[] = readDir(readPath);
    ...
    while (queue.length !== 0) {
      ...
      directoryFiles.forEach(directoryItem => {
        if (directoryItem.dirent.isDirectory()) {
          ...
          // scan files
          const targetFiles = [];
          fs.readdirSync(directoryItem.path, { withFileTypes: true }).forEach((fileDirent: Dirent) => {
            if (fileDirent.isFile() && accessExtension.find(accessExtension =>  nodePath.extname(fileDirent.name).toLowerCase() === accessExtension.toLowerCase()) !== undefined){
              targetFiles.push({
